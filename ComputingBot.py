...
async def on_ready():
    ...
    activity=discord.Activity(name='Pokimane',type=discord.ActivityType.watching),status=discord.Status.dnd)
...
async def breed(ctx, *, something):
    async with aiohttp.ClientSession() as cs:
        ...
        async with cs.get(url) as resp:
            js = await resp.json() 
            if len(js)==3:
                await ctx.send(":dog: Breed not found! Try another one (Use lowercase). :dog:")
            else:
...
@bot.command()
async def git(ctx):
    await ctx.send(ctx.author.mention + " " + "https://github.com/TheTripleJHD/ComputingBot/ComputingBot.py")
